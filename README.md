...
- **Concrete Outcomes**: Predicted humidity and settled temperature after the recommended session (both shown as headline numbers), plus estimated liters of water removed
- **Session Planner**: Slider (or scrub directly on the chart) to pick any venting duration up to 8 h — the prediction is simulated in 5-minute steps against the hourly forecast, so a session that stretches into changing weather is priced correctly, with red bands marking condensation-risk stretches and amber bands marking cellar-warming stretches. The humidity curve is judged at the cellar's normal temperature: hot afternoon air can't disguise its moisture load behind a temporarily warm (= lower-reading) RH percentage
- **Two-Node Wall Model**: The masonry is simulated alongside the air — old damp walls buffer moisture (re-releasing it within ~2 h of any venting), warm slowly during long warm sessions, and emit more when warmed. Water extraction is accounted **cumulatively through the window** hour by hour, not as a before/after air snapshot; cellar-warming stretches are marked amber and their settled-temperature cost is reported
- **Confidence-Gated Recommendation**: At every simulated step the same per-hour gates as the "now" verdict (condensation on cold walls, meaningful drying vs the *updated* basement air) are re-applied on a **pessimistic forecast path** — the recommendation covers only the stretch that helps even if the forecast turns out moisture-optimistic. Forecast uncertainty grows with lead time and grows extra when a forecast promises much drier air than is measured right now (regression toward observation); the chart shows the central estimate inside the uncertainty band. No moisture assumptions about warm or cold hours — a genuinely dry föhn hour can pass the moisture gates, an uncertain "dry afternoon" promise cannot. Temperature is a separate, explicit objective: hours that would push the cellar above the ideal 15–20 °C band (or heat a cellar already above it) are never *recommended*, only simulated as what-if. Verdict/advice re-evaluate live for whatever duration the slider selects, burst-capped in freezing weather
- **Temperature Estimate**: A lumped thermal model predicts the air temperature dip while venting and the rebound after closing — massive masonry stores far more heat than the air (~0.4 kWh/K air vs tens of kWh/K in the first centimeters of wall), so only a small fraction of the swing persists after ~1 h; the predicted humidity is evaluated at that settled temperature
