...
- **Confidence-Gated Recommendation**: At every simulated step the same per-hour gates as the "now" verdict (condensation on cold walls, meaningful drying vs the *updated* basement air) are re-applied on a **pessimistic forecast path** — the recommendation covers only the stretch that helps even if the forecast turns out moisture-optimistic. Forecast uncertainty grows with lead time and grows extra when a forecast promises much drier air than is measured right now (regression toward observation); the chart shows the central estimate inside the uncertainty band. No categorical rules about warm or cold hours — a genuinely dry föhn hour can qualify, an uncertain "dry afternoon" promise cannot. Verdict/advice re-evaluate live for whatever duration the slider selects, burst-capped in freezing weather
