...
- **Two-Node Wall Model**: The masonry is simulated alongside the air — old damp walls buffer moisture (re-releasing it within ~2 h of any venting, so marathon sessions rebound and short repeated bursts win), warm slowly during long warm sessions, and emit more when warmed. Water extraction is accounted **cumulatively through the window** hour by hour, not as a before/after air snapshot — and venting with cellar-heating air is never recommended, only simulated as what-if
- **Simulation-Driven Recommendation**: The recommended duration is the shortest session within 1.5 %RH of the best achievable result over the 8 h horizon (truncated before condensation-risk stretches, burst-capped in freezing weather), and the verdict/advice re-evaluate live for whatever duration the slider selects
