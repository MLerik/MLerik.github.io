...
- **Session Planner**: Slider (or scrub directly on the chart) to pick any venting duration up to 8 h — the prediction is simulated in 5-minute steps against the hourly forecast, so a session that stretches into changing weather is priced correctly, with red bands marking condensation-risk stretches
